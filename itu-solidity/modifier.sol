...
contract StructEnum {
    ...
    function updateZip(uint256 _orderId, string memory _zipCode) onlyCustomer(_orderId) checkOrderId(_orderId) increaseTransaction   external {
        // require(orders.length > _orderId, "Not a valid order id.");        
    ...
    modifier checkProducts(uint256[] memory _products) {
        require(_products.length > 0, "No products.");
        _;//modifier'i cagirdigimiz fonksiyonun body'sini calistirmak icin pre-post ayarlamasi yapar
    }
